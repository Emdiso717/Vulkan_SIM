...
// SRS - Metal layer is defined externally when using iOS/macOS
// displayLink-driven examples project
...
//     displayLinkOutputCb()
  ...
  while (true) {
    ...
    if (prepared) {
      ...
      float fpsTimer =
          std::chrono::duration<double, std::milli>(tEnd - lastTimestamp)
              .count();
      if (fpsTimer > 1000.0f) {
      ...
      if (camera.type != Camera::CameraType::firstperson) {
        ...
        if (std::abs(gamePadState.axisRight.y) > deadZone) {
          camera.translate(
              glm::vec3(0.0f, 0.0f, gamePadState.axisRight.y * 0.01f));
        }
      } else {
        camera.updatePad(gamePadState.axisLeft, gamePadState.axisRight,
                         frameTimer);
      }
    }
  }
...
#else
  // With MoltenVK, use layer settings extension to configure it with common
  // project config settings Other implementations like lavapipe and KosmicKrisp
  // do not need this
  uint32_t extCount = 0;
  vkEnumerateInstanceExtensionProperties(nullptr, &extCount, nullptr);
  if (extCount > 0) {
    ...
    if (vkEnumerateInstanceExtensionProperties(
      ...
      for (VkExtensionProperties &extension : extensions) {
        if (std::strcmp(extension.extensionName,
          ...
          // Configure MoltenVK to use to use a dedicated compute queue (see
          // compute[*] and timelinesemaphore samples)
          VkLayerSettingEXT layerSetting{
              .pLayerName = "MoltenVK",
              .pSettingName = "MVK_CONFIG_SPECIALIZED_QUEUE_FAMILIES",
              .type = VK_LAYER_SETTING_TYPE_BOOL32_EXT,
              .valueCount = 1};
          // Make this static so layer setting reference remains valid after
          // leaving constructor scope
...
bool VulkanExampleBase::initVulkan() {
  // Instead of checking for the command line switch, validation can be forced
  // via a define
...
int32_t VulkanExampleBase::handleAppInput(struct android_app *app,
  ...
  if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY) {
    ...
    case AKEYCODE_BUTTON_START:
      ...
      break;
    default:
      vulkanExample->keyPressed(
          keyCode); // handle example-specific key press events
      break;
    };
...
// SRS - Dispatch rendering loop onto a queue for max frame rate concurrent
// rendering vs displayLink vsync rendering
//     - vsync command line option (-vs) on macOS now works like other platforms
//     (using VK_PRESENT_MODE_FIFO_KHR)
dispatch_group_t concurrentGroup;

- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
  [NSApp activateIgnoringOtherApps:YES]; // SRS - Make sure app window launches
                                         // in front of Xcode window
  ...
  // SRS - When benchmarking, set up termination notification on main thread
  // when concurrent queue completes
...
// SRS - Tell rendering loop to quit, then wait for concurrent queue to
// terminate before deleting vulkanExample
...
- (void)viewDidMoveToWindow {
  ...
  // SRS - Disable displayLink vsync rendering in favour of max frame rate
  // concurrent rendering
  //     - vsync command line option (-vs) on macOS now works like other
  //     platforms (using VK_PRESENT_MODE_FIFO_KHR)
...
- (void)keyDown:(NSEvent *)event {
  ...
  default:
    vulkanExample->keyPressed(
        event.keyCode); // handle example-specific key press events
    break;
  }
}
...
- (void)scrollWheel:(NSEvent *)event {
  ...
  vulkanExample->camera.translate(glm::vec3(
      0.0f, 0.0f,
...
void VulkanExampleBase::windowDidResize() { resizing = false; }
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
...
void VulkanExampleBase::handleEvent() {
  ...
  while (!screen_get_event(screen_context, screen_event, paused ? ~0 : 0)) {
    ...
    case SCREEN_EVENT_KEYBOARD:
      rc = screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_FLAGS,
                                        &keyflags);
      if (rc) {
      ...
      rc =
          screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_SYM, &val);
      if (rc) {
    ...
    case SCREEN_EVENT_PROPERTY:
      rc = screen_get_event_property_pv(screen_event, SCREEN_PROPERTY_WINDOW,
                                        (void **)&win);
      if (rc) {
      ...
      rc = screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_NAME,
                                        &val);
      if (rc) {
      ...
      if (win == screen_window) {
        ...
        case SCREEN_PROPERTY_SIZE:
          ...
          if (rc) {
            printf("Cannot get SCREEN_PROPERTY_SIZE of the window in the "
                   "event! (%s)\n",
                   strerror(errno));
            fflush(stdout);
            quit = true;
    ...
    case SCREEN_EVENT_POINTER:
      rc = screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_BUTTONS,
                                        &val);
      if (rc) {
      ...
      rc = screen_get_event_property_iv(screen_event,
                                        SCREEN_PROPERTY_MOUSE_WHEEL, &val);
      if (rc) {
      ...
      rc = screen_get_event_property_iv(screen_event, SCREEN_PROPERTY_POSITION,
                                        pos);
      if (rc) {
...
void VulkanExampleBase::setupWindow() {
  ...
  /* Set window caption */
  screen_set_window_property_cv(screen_window, SCREEN_PROPERTY_ID_STRING,
                                strlen(idstr), idstr);

  /* Setup VULKAN usage flags */
  rc = screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_USAGE,
                                     &usage);
  if (rc) {
  ...
  if ((width == 0) || (height == 0) || (settings.fullscreen) ||
      use_window_size) {
    rc = screen_get_window_property_iv(screen_window, SCREEN_PROPERTY_SIZE,
                                       size);
    if (rc) {
    ...
    size[0] = width;
    size[1] = height;
    rc = screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_SIZE,
                                       size);
    if (rc) {
  ...
  if (screen_pos_set) {
    rc = screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_POSITION,
                                       screen_pos);
    if (rc) {
  ...
  if (screen_pipeline_set) {
    rc = screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_PIPELINE,
                                       &screen_pipeline);
    if (rc) {
  ...
  if (screen_zorder_set) {
    rc = screen_set_window_property_iv(screen_window, SCREEN_PROPERTY_ZORDER,
                                       &screen_zorder);
    if (rc) {
...
#if defined(_WIN32)
void VulkanExampleBase::OnHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam,
                                        LPARAM lParam) {};
#endif
