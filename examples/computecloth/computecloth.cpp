 ...
 *
 * A compute shader updates a shader storage buffer that contains particles held
 * together by springs and also does basic collision detection against a sphere.
 * This storage buffer is then used as the vertex input for the graphics part of
 * the sample
 *
...
public:
  ...
  // This will be set to true, if the device has a dedicated queue from a
  // compute only queue family With such a queue graphics and compute workloads
  // can run in parallel, but this also requires additional barriers (often
  ...
  // We put the resource "types" into structs to make this sample easier to
  // understand

  // We use two buffers for our cloth simulation: One with the input cloth data
  // and one for outputting updated values The compute pipeline will update the
  // output buffer, and the graphics pipeline will it as a vertex buffer
  ...
  void prepareCompute() {
    ...
    computePipelineCreateInfo.stage =
        loadShader(getShadersPath() + "computecloth/cloth.comp.spv",
  ...
  void buildComputeCommandBuffer() {
    ...
    for (uint32_t j = 0; j < iterations; j++) {
      ...
      // Don't add a barrier on the last iteration of the loop, since we'll have
      // an explicit release to the graphics queue
      if (j != iterations - 1) {
