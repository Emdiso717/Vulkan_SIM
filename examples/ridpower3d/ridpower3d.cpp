// Power-measurement executable: RID only.
...
// A rendered frame advances this fixed simulation time; it may be split into
// multiple RID substeps for stability.
...
// A standalone, RID-only power-measurement executable.
...
public:
  ...
  // This is a solver-agnostic kinematic boundary pass.
  ...
  void OnUpdateUIOverlay(vks::UIOverlay *) override {
    ImGui::TextUnformatted("Solver: RID (power measurement)");
...
private:
  ...
  void updateSimulationParams() {
    ...
    params.ground = glm::vec4(0.0f);
    std::memcpy(ridCompute.parameters.mapped, &params, sizeof(params));
  }
